...
clear RatSniff_Pxx_Mat Groom_Pxx_Mat Baseline_Pxx_Mat RatSniff_Pxx_Mat3 Groom_Pxx_Mat3 Baseline_Pxx_Mat3 RatSniff_Cxy_Mat Groom_Cxy_Mat Baseline_Cxy_Mat
close all;
...
%% Rat 1
Groom = [Groom_Rat1_amyg]; % Groom_Rat1_amyg_amyg
Baseline = [Baseline_Rat1_amyg];
RatSniff = [RatSniff_Rat1_amyg];
%Measure size of all
G_size = size(Groom_Rat1_amyg, 1);
B_size = size(Baseline_Rat1_amyg, 1);
RS_size = size(RatSniff_Rat1_amyg, 1);
...
%     RatSniff_Pxx_Mat(i,:,:) = Pxx;
    LFP_amyg = RatSniff_Rat1_amyg(i,:);
    LFP_ca2 = RatSniff_Rat1_ca2(i,:);
    [Cxy,F] = mscohere(LFP_amyg,LFP_ca2,w,overlap,NFFT,Fs);
    RatSniff_Cxy_Mat(i,:,:) = Cxy;
...
%     Groom_Pxx_Mat(i,:,:) = Pxx;
    ...
    LFP_amyg = Groom_Rat1_amyg(i,:);
    LFP_ca2 = Groom_Rat1_ca2(i,:);
    [Cxy,F] = mscohere(LFP_amyg,LFP_ca2,w,overlap,NFFT,Fs);
    Groom_Cxy_Mat(i,:,:) = Cxy;
...
%     Baseline_Pxx_Mat(i,:,:) = Pxx;
    ...
    LFP_amyg = Baseline_Rat1_amyg(i,:);
    LFP_ca2 = Baseline_Rat1_ca2(i,:);
    [Cxy,F] = mscohere(LFP_amyg,LFP_ca2,w,overlap,NFFT,Fs);
    Baseline_Cxy_Mat(i,:,:) = Cxy;
...
figure(2)
subplot(311)
plot(F, mean(Baseline_Cxy_Mat, 1), 'k', 'LineWidth', 2)
hold on
plot(F, mean(Groom_Cxy_Mat, 1), 'g', 'LineWidth', 2)
plot(F, mean(RatSniff_Cxy_Mat, 1), 'r', 'LineWidth', 2)
% xlim([50 100])
% ylim([0 .0018])
xlim(myXlims)
ylim(myYlims)
...
%% Rat 2
% Groom = Groom_Rat2_amyg;
% Baseline = Baseline_Rat2_amyg;
% RatSniff = RatSniff_Rat2_amyg;
%Measure size of all
G_size = size(Groom_Rat2_amyg, 1);
B_size = size(Baseline_Rat2_amyg, 1);
RS_size = size(RatSniff_Rat2_amyg, 1);


for i = 1:RS_size
    LFP_amyg = RatSniff_Rat2_amyg(i,:);
    LFP_ca2 = RatSniff_Rat2_ca2(i,:);
...
for i = 1:G_size
    LFP_amyg = Groom_Rat2_amyg(i,:);
    LFP_ca2 = Groom_Rat2_ca2(i,:);
%     LFP = Groom(i,:);
%     [Pxx,F] = pwelch(LFP,w,overlap,NFFT,Fs);
%     Groom_Pxx_Mat(i,:,:) = Pxx;
...
for i = 1:B_size
    LFP_amyg = Baseline_Rat2_amyg(i,:);
    LFP_ca2 = Baseline_Rat2_ca2(i,:);
%     LFP = Baseline(i,:);
%     [Pxx,F] = pwelch(LFP,w,overlap,NFFT,Fs);
%     Baseline_Pxx_Mat(i,:,:) = Pxx;
...
% %% Both
% Groom = [Groom_Rat1_amyg; Groom_Rat2_amyg];
% Baseline = [Baseline_Rat1_amyg; Baseline_Rat2_amyg];
% RatSniff = [RatSniff_Rat1_amyg; RatSniff_Rat2_amyg];
% %Measure size of all
% G_size = size(Groom_Rat1_amyg, 1) + size(Groom_Rat2_amyg, 1);
% B_size = size(Baseline_Rat1_amyg, 1) + size(Baseline_Rat2_amyg, 1);
% RS_size = size(RatSniff_Rat1_amyg, 1) + size(RatSniff_Rat2_amyg, 1);
% 
% for i = 1:RS_size
%     LFP = RatSniff(i,:);
%     [Pxx,F] = pwelch(LFP,w,overlap,NFFT,Fs);
...
% hold on
% plot(F, mean(Groom_Pxx_Mat3, 1), 'g', 'LineWidth', 2)
% plot(F, mean(RatSniff_Pxx_Mat3, 1), 'r', 'LineWidth', 2)
% xlim(myXlims)
% ylim(myYlims)
% title('Both')
% xlabel('milliseconds')
